...
$$\sum_{\alpha\in\{0,1\}^k}|Pr[x_{i_1}x_{i_2}...x_{i_k}]-2^{-k}| \le \epsilon \Rightarrow |Pr[x_{i_1}x_{i_2}...x_{i_k}]-2^{-k}|\le \epsilon $$
	...
	\noindent First, we will use following family of k-wise $\delta$-dependent function:\\
	...
	\noindent To prove Corollary 2.3.1, we will need to understand the following fact:\\	
	...
	\noindent To prove Fact 2.3.2, we will need to use the following two lemma and corollary. For the proofs of the Lemma 2.3.2.1 and Corollary 2.3.2.2, please refer to Appendix. \\
	\noindent\textbf{Lemma 2.3.2.1 (Vazirani):} Let $S_n\subseteq{0,1}^n$ be a sample space that is $\epsilon$-biased with respect to linear tests of size at most k. Then, the sample space $S_n$ is $((1-2^{-k})\epsilon,k)$-independent(in max form), and $(2^k-1)^{\frac12}\epsilon$-away(in $L_1$ norm) from k-wise independent.\\
...
\noindent Regards to this construction, we have following claim:\\
...
\noindent Now we will elaborate the connection between the construction above and Corollary 2.3.1. We will show that this construction will meet the space and time requirement specified in Corollary 2.3.1\\

\noindent\textbf{Connection to Corollary 2.3.1:}\\
...
\noindent Now, after proved Lemma 2.3.1, we will state a bound for random variables with limited independence. We will introduce Lemma 2.2 of paper titled \textit{Randomness Efficient Oblivious Sampling}. This lemma is very important in proving our construction. We first state following Lemma.\\
...
\noindent To prove Lemma 2.3.2, we will need a Lemma conducted by Bellare(1994) from the paper titled \textit{Randomness Efficient Oblivious Sampling}. It is named as the First t-wise Independent Tail Inequality in the original paper and labeled as Lemma 2.3.3 here. \\
...
Since we have proven Theorem 3.1, it implies that with high probability at least $1-d/n^{c+1} > 1-1/n^c$, a randomly selected function from the hash family from our construction has a maximum load of $O(logn/loglogn)$ while guaranteeing each function can be described in $o(log^2 n/loglogn)$. 
	...
	This section introduces a few more lemmas and corollaries that we used in the proof of the construction. However, these proofs are study result of other papers, hence, we are directly introducing them as an appendix. \\
	 
	\noindent\textbf{Lemma 2.3.2.1 (Vazirani):} Let $S_n\subseteq{0,1}^n$ be a sample space that is $\epsilon$-biased with respect to linear tests of size at most k. Then, the sample space $S_n$ is $((1-2^{-k})\epsilon,k)$-independent(in max form), and $(2^k-1)^{\frac12}\epsilon$-away(in $L_1$ norm) from k-wise independent.\\
	
	\noindent\textbf{Proof of Lemma 2.3.2.1 (Vazirani):} \\
	...
	and we know that 
	$$ \sum_{\alpha} p^2_{\alpha} = 2^{-k} \sum_{\beta}c^2_{\beta}$$ 
	Now with these information, we have 
	$$ |p_{\alpha} - 2^{-k}| = 2^{-k} \sum_{\beta}\phi_{\beta}(\alpha)c_{\beta} \le (1-2^{-k})\epsilon, $$
	...
	$$\sum_{\alpha}|p_{\alpha} - 2^{-k}| \le 2^{k/2}(\sum_{\alpha} (p_{\alpha} - 2^{-k})^2)^{1/2} = 2^{k/2}(2^{-k}\sum_{\beta \ne 0} c^2_{\beta})^{1/2} \le (2^k-1)^{1/2}\epsilon. $$
	This completes the second part of the Lemma. Hence we have completed the proof of Lemma 2.3.2.1. $\hfill\blacksquare$ \\
	
	\noindent In particular, Lemma 2.3.2.1 will imply Corollary 2.3.2.2. It's clear to see how this following Corollary is a transformation of Lemma 2.3.2.1 and hence, we will not show the steps. \\
